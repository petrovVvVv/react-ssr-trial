...
const findPageInJson = (url) => find(compose(equals(url), head, Object.keys))
...
app.use('*', async (req, res) => {
    ...
    if (hasStaticPageForUrl) {
        ...
        indexHTML = indexHTML
            .replace(`<html lang="bg" />`, `<html lang="${pageData.language}" />`)
            .replace(
                `<meta name="description" content="Page Description">`,
                `<meta name="description" content="${pageData.meta.meta_description}">`
            )
            .replace(`<title>Website Title</title>`, `<title>${pageData.meta.meta_title}</title>`)
    }
